...
class SogouImgDownloader(object):

    """搜狗图片下载工具，目前只支持单个关键词"""

    re_objURL = re.compile(r'"pic_url":"(.*?)"')
    ...
    def __init__(self, word, dirpath=None, processNum=30):
        ...
        self.session.headers = SogouImgDownloader.headers
        self.queue = Queue()
    ...
    def __buildUrls(self):
        ...
        html = self.session.get(url.format(word=word, pn=0), timeout = 15).text
        maxNum = 100000 
        urls = [url.format(word=word, pn=x)
                for x in range(0, maxNum + 1, 48)]
        print (urls)
